...
public class MemberControllerTests {
    ...
    void t005() throws Exception {
        ...
        HttpSession session = mvcResult.getRequest().getSession(false);// 원래 getSession 을 하면, 만약에 없을 경우에 만들어서라도 줌, false 는 없으면 만들지 말라는 뜻
    ...
    @Test
    // @Rollback(value = false) // DB에 흔적이 남는다.
    @DisplayName("로그인 후에 내비게이션 바에 로그인한 회원의 username")
